...
const BackgroundEffects = () => {
  return (
    <div className="fixed inset-0 -z-0 pointer-events-none overflow-hidden bg-background">
      {/* Animated Gradient Orbs - Optimized using pure radial gradients instead of CSS blur to eliminate lag */}
      <div className="absolute top-[-20%] left-[-10%] w-[50vw] h-[50vw] animate-blob" style={{ background: 'radial-gradient(circle, rgba(193,155,118,0.15) 0%, transparent 70%)' }}></div>
      <div className="absolute top-[30%] right-[-10%] w-[60vw] h-[60vw] animate-blob" style={{ animationDelay: "2s", background: 'radial-gradient(circle, rgba(140,82,48,0.15) 0%, transparent 70%)' }}></div>
      <div className="absolute bottom-[-20%] left-[20%] w-[50vw] h-[50vw] animate-blob" style={{ animationDelay: "4s", background: 'radial-gradient(circle, rgba(74,49,32,0.2) 0%, transparent 70%)' }}></div>
      
      {/* Premium Noise Texture - Optimized without expensive blend modes */}
...
const Hero = () => {
  ...
  return (
    <section ref={container} className="relative h-screen flex items-center justify-center overflow-hidden">
      <div className="absolute inset-0 bg-background z-0 hero-bg">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(193,155,118,0.15)_0%,transparent_60%)]"></div>
        <div className="absolute top-0 left-0 w-full h-full bg-[url('https://images.unsplash.com/photo-1560066984-138dadb4c035?auto=format&fit=crop&q=80')] bg-cover bg-center opacity-5 mix-blend-overlay"></div>
      </div>
...
const Reality = () => {
  ...
  return (
    <section ref={container} className="py-32 px-4 relative z-10 bg-transparent">
      <div className="max-w-7xl mx-auto">
        <div className="mb-20 text-center md:text-left grid grid-cols-1 md:grid-cols-2 gap-10 items-end">
          <div>
            <h2 className="font-display text-4xl md:text-6xl font-bold mb-6">Реальність <br/><span className="text-textMuted font-light">beauty-індустрії</span></h2>
          </div>
          <div>
...
const MasterTrap = () => {
    ...
    return (
      <section ref={container} className="py-40 px-4 relative flex items-center justify-center min-h-[80vh] overflow-hidden">
        <div className="circle-bg absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] rounded-full border border-primary/20 bg-primary/5 blur-3xl -z-10"></div>
        <div className="max-w-4xl mx-auto text-center z-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-left">
              {['Перевантаження', 'Нестабільний дохід', 'Хаос у процесах', 'Залежність бізнесу'].map((t, i) => (
    ...
    return (
      <section className="py-32 px-4 my-20 bg-surface/80 text-white text-center border-y border-white/5 relative z-10 glass-card mx-4 rounded-3xl">
          <div className="max-w-4xl mx-auto">
              ...
              <p className="text-xl md:text-2xl mt-12 text-gray-600 font-light">
...
const HorizontalCamps = () => {
  ...
  const camps = [
    {
      num: 1, title: "Стан власника", subtitle: "Чому майстри залишаються майстрами",
      items: ["Мислення працівника vs власника", "Енергія, дисципліна і гроші", "Фінансовий потолок майстра", "Формування візії бізнесу"]
    },
    {
      num: 2, title: "Фінансова формула", subtitle: "Як працюють гроші у beauty",
      items: ["Формула доходу", "Ячейка майстра/салону", "Середній чек та повторні продажі", "Бізнес-метрики: ROI, LTV, NPS"]
    },
    {
      num: 3, title: "Система і команда", subtitle: "Як перестати бути вузьким горлом",
      items: ["Делегування процесів", "Ролі в команді", "Стандарти сервісу", "Система управління студією"]
    },
    {
      num: 4, title: "Масштаб і стратегія", subtitle: "Як масштабувати бізнес",
      items: ["Продуктові напрямки (навчання)", "Стратегія на 12 місяців", "Розвиток бренду", "Робота з ризиками"]
    }
  ];
  ...
  return (
    <section ref={sectionRef} className="h-screen bg-transparent flex flex-col justify-center overflow-hidden relative z-10">
      ...
      <div ref={scrollRef} className="flex gap-10 px-10 w-[400vw] lg:w-[150vw]">
        {camps.map((camp, i) => (
          <div key={i} className="flex-shrink-0 w-[85vw] md:w-[60vw] lg:w-[35vw] glass-card p-10 flex flex-col h-[500px]">
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {[
                        ...
                        <div key={i} className="glass-card p-8 flex items-start gap-4 hover:border-primary/50 transition-colors">
                ...
                <div className="mt-20 p-12 glass-card bg-primary/10 border-primary/30 text-center rounded-3xl relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8 relative z-10">
                        {['Дохід у 2-3 рази', 'Особистий час', 'Системність', 'Нова роль'].map((t, i) => (
                            <div key={i} className="text-center">
                                <div className="text-primary mb-2 flex justify-center">
                                    {i === 0 ? <TrendingUp size={32}/> : i === 1 ? <Target size={32}/> : i === 2 ? <Award size={32}/> : <Users size={32}/>}
                                </div>
...
const Footer = () => {
  return (
    <footer className="py-40 px-4 text-center border-t border-white/5 relative bg-transparent z-10">
